...
The single server, client protocol, and run view remain the correct base. Version 0.16.0 still uses a short activity lease, clears terminal session display too soon, and did not preserve all session features during the cut. [Unify workflow messages and restore server-side behavior](2026-09-02-unify-workflow-messages-plan.md) is the approved follow-up. It replaces the activity lease with Pi turn events and gives all Pi-bound workflow content one shared message path.

## Goal

Give every live Pi Workflows interface one source of truth.

One package-owned server will own live SQLite state and produce one versioned run view. The Pi widget, status line, workflow controls, Herdr actions, CLI, and Rust `piw` viewer will use the same client protocol and the same run view. No production client will read or interpret live SQLite state.

This is one client design, not three separate repairs:

```text
one server + one WorkflowClient v1 protocol + one run view + several renderers
```
...
- A matching installed `piw` rejected a live database that the TypeScript server accepted because Rust contained an older copied application version and DDL digest.
...
The Pi extension opens SQLite twice during one refresh. It first finds the origin session reservation and then loads the run from a second database snapshot. It maps durable workflow state directly to a widget label. The server cannot add exact origin-session model activity to that result.
...
The extension can associate its own delivered workflow message with the matching origin-session model turn. It can report that activity to the server without changing Pi core, reading private state, or changing Pi session files. No required public capability is missing.

## Boundaries

### In scope

- Replace the current server request, direct SQLite view, and replay client paths with one versioned live client protocol.
- Make the server the only production process that opens the active SQLite database.
- Add one server-produced live run view with one effective display status and allowed controls.
...
- Make local and remote `piw` use the same server protocol and projection.
- Move active database status, verification, backup, and prune commands through the server.
...
Replace the earlier request, response, and replay schemas with one logical protocol named `pi-workflows.client.v1`.
...
An `interaction.submit` request stays open while the supervised child validates the submitted value. Its one response settles only after the durable submission becomes `accepted`, `adopted`, or `rejected`. If the connection fails, repeating the same request ID and payload waits for and returns the same durable outcome. A local tool abort stops waiting without cancelling the server command. A later retry uses a new transport request ID with the same durable idempotency and submission IDs, so a late response cannot settle the retry. Clients do not poll SQLite for validation results.
...
- on-demand server startup and connection without a durable status probe;
- explicit status requests when a caller asks for server status;
...
The server produces `pi-workflows.run-view.v1` from one consistent server-side read. It includes:
...
The origin-session response also contains read-only notification and turn availability. The shared delivery coordinator issues a claim only when the matching fact is true. A claimed terminal turn loads its exact run by ID instead of using the latest run in the session. Its idle poll does not write an empty claim or server-status command.

### One status reducer

Only the server computes the effective display status.
...
A parked queue is not enough to display `paused`. Only the durable pause flag permits that label and icon. A renderer does not inspect workflow rows to override the server result.
...
The server accepts activity only when the durable pending interactive request and its recorded delivery or presentation entry match the session, run, request, and delivery. Presentation-claim settlement does not end the request, so valid activity can continue while the model turn runs. Repeated reports are idempotent. A stale sequence, replaced delivery, or wrong session is rejected.

The server keeps activity in memory with a short renewable lease tied to the client connection. The first report on each connection is `started`; only later reports on that same connection are refreshes. One constants module under `src/client/` owns both the refresh period and lease duration. The refresh period must be shorter than the lease duration, and the lease duration must bound how long a dead client can leave a false `running` display. The server clears activity on the matching settled event, connection loss, or lease expiry. A missing report falls back to durable `waiting`. It never creates a false `paused` or `running` state.

Activity is display evidence only. The server requires the deterministic `interaction:<request-id>` delivery ID and keys one overlay by connection and request. A caller-supplied alternate label cannot create a second overlay. Activity cannot renew a run claim, settle an interaction, change a workflow state, or authorize a control command. Durable workflow correctness does not depend on it.
...
Local `piw` connects to the package-owned server, subscribes to run views, and fetches pages through `pi-workflows.client.v1`. When the socket is absent, Rust may start the server only by executing the installed `pi-workflows server start` command. Rust does not reimplement server launch, locking, epochs, or readiness. If that command is unavailable or fails, `piw` stops with the direct install or startup instruction.

`piw serve` stops reading SQLite. It becomes a loopback-only WebSocket relay for the same logical protocol. Remote clients continue to use an SSH tunnel. The relay opens one server socket connection for each WebSocket connection and couples their lifecycles one to one. It carries frames only. It does not multiplex clients, translate run state, keep another projection, or retain activity after either side closes.

Remove the selectable local SQLite source and the copied TypeScript application version and DDL digest from Rust. The Rust viewer has no SQLite mode, including for backups. If the server and viewer protocol do not match, `piw` stops with one direct package-version instruction. It must not suggest deleting live state and must not fall back to SQLite.
...
- Do not change the SQLite schema unless implementation proves that the server view needs a durable field. The activity overlay needs no durable field.
...
- Extend the version-1 `server_commands` operation set in place so backup and prune receipts are durable.
...
**Change:** Define the one envelope, operation names, outcomes, subscriptions, run-view snapshots, patches, pages, activity reports, maintenance requests, safe errors, and protocol handshake. Keep foreground server-start retry timers referenced until success or timeout; unreference only background reconnect timers. End a backpressured write wait on connection close, socket error, or request cancellation. Replace the server and replay schema identifiers. Add one accepted and rejected fixture corpus used by TypeScript and Rust.
...
### 2. Make the server the only live database reader

**Location:** `src/server/`, `src/state/`, resource manager stores, and state maintenance commands.

**Change:** Add server handlers for atomic origin-session view lookup, run-list and run-view snapshots, revision subscriptions, bounded pages, and active database maintenance. Keep projection reads and writes in the server. A view read must resolve the session reservation, run, durable display facts, and presentation revision from one consistent read boundary. Persist generated large view values in the run-scoped content table under their digest and media type before advertising their references. Do not reuse general state blobs or allow another run to read the content. Store the complete original workflow definition, not its escaped projection. Externalize large replay checkpoints and make both clients resolve them. Route state-changing Pi extension commands through the durable retry path. Give the CLI one stable client identity and one fresh key for each backup or applied prune invocation. Reuse that key only for the invocation's automatic reconnect retry, with a new request ID. Keep an in-flight maintenance operation alive after disconnect, store its accepted or rejected command receipt before response, wait for it during server shutdown, and adopt an exact retry. Wait for socket drain before sending another snapshot, and remove every subscription kind explicitly when its client unsubscribes.
...
### 3. Add the server status reducer and activity overlay

**Location:** a server-owned view module under `src/server/` or `src/viewer/`, with no Pi import.
...
**Change:** Make the protocol client the only Rust source. Remove the default active database path, all Rust `ProjectionReader` and backup-reader paths, copied application version, copied DDL digest, and direct-reader command selection. Start the server only through the installed TypeScript CLI. Make `piw serve` map each loopback WebSocket connection to one server socket connection and relay the same client protocol. Use the Unix socket on Unix and the package-derived named pipe on Windows. Drop old run pages, content requests, and artifacts when selection moves to another run.

**Verification:** A database created by the TypeScript package is visible in local and remote `piw` through the server. A protocol mismatch gives a package-version error. A valid live database never produces a Rust DDL mismatch. No `piw` live mode opens `state.sqlite`.

### 7. Remove split paths and update documentation

**Location:** old server/replay protocol code, obsolete viewer readers, `docs/WORKFLOW_SERVER.md`, `docs/SQLITE_STATE.md`, `docs/LIVE_REPLAY_PROTOCOL.md`, README usage, and package contents.
...
- If the server cannot start or connect, clients show `unavailable` with one bounded next action. They do not display a cached run as current.
- If a client disconnects, the server expires its activity and subscriptions. Reconnection requests a patch from the last accepted revision or receives a bounded snapshot.
- If a subscriber falls behind retained revisions, only that subscriber receives a new bounded snapshot.
- If an activity report is stale or mismatched, the server rejects it and keeps the durable view.
...
- If the live SQLite schema is truly incompatible, the server fails before mutation with the standard backup-and-reset instruction and leaves all files untouched.
...
- Start one server and connect extension, CLI, and Rust clients to the same run.
...
- Restart the server and recover subscriptions without a second database reader.
- Run the loopback WebSocket relay and compare its messages with local socket messages.
- Verify that active-state maintenance commands go through the server.
- Verify that only the TypeScript inactive-backup verifier can open SQLite outside the server and that it cannot select the active path.
...
Before release, test the packed package in a new Pi session and a disposable Herdr tab. Confirm that the package starts its server on demand and does not install an operating-system service.
...
- the server is the only production process that opens active SQLite state;
- all live clients use `pi-workflows.client.v1`;
- the server produces one canonical run view and one display status;
