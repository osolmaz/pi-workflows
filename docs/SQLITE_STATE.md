...
`session_message_revisions` stores one counter for each target session, and three triggers on `workflow_messages` move it for every insert, update, and delete. The counter covers a direct SQL writer as well as the message store, so the session view cache key reads one row instead of aggregating the session's stored messages.
