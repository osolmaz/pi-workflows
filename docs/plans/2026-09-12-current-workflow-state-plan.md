...
| A stale snapshot renders but cannot authorize            | The extension keeps the last view, adds the `stale` marker, and refuses commands and branch reports for a stale session id. It also fences the message coordinator, so no Pi turn starts from a snapshot the server no longer confirms: `test/workflow-message-coordinator.test.ts` "sends nothing from a snapshot whose subscription failed" fails without the fence, and `test/extension.test.ts` "fences workflow delivery when the session subscription is lost" proves the wiring.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
