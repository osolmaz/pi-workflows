...
```text
~/.pi/agent/workflows/server/server.sock
```

On Windows, the same client uses the package-derived `\\.\pipe\pi-workflows-<state-directory-hash>` named pipe. Local `piw`, `piw serve`, and the TypeScript client derive the same endpoint from the workflow state directory. Operating systems limit one local socket path: Linux allows 107 bytes, and macOS and the BSDs allow 103 bytes. A longer state directory is a startup blocker: the server refuses to listen and each client reports the path, the measured length, and the limit before it probes or starts a server, instead of waiting for a socket that can never be reached. A foreground TypeScript cold start keeps its retry wait referenced until the detached server becomes ready or the start deadline expires. Only background reconnect timers are unreferenced.

Each message is one canonical JSON object followed by a newline. TypeScript and Rust use the same ECMAScript number formatting and UTF-16 object-key order, including for arbitrary workflow JSON. A message can be at most 1 MiB. The server writes each frame through one measured writer, which encodes the message once and measures the complete frame. A frame above the limit never closes the connection: a response becomes a bounded `rejected` response, and an event fails only its own subscription. Every server-built frame is bounded well below the limit as well, including the workflow name in a run list summary and in the bounded run view. Unknown envelope fields, non-canonical JSON, and invalid framing close only the offending connection. If socket backpressure delays a client write, connection close, socket error, or request cancellation ends the wait instead of leaving the request pending.
...
- `event` carries a revisioned run list, run snapshot, run page, session snapshot, or unavailable condition.
...
The server also owns the advisory `display.controls` strings. The current values are `pause`, `resume`, `cancel`, `answer`, `human-answer`, `update`, `submit`, and `review`. TypeScript defines this set once. Rust accepts any string in this display-only field, including a future value. `piw` does not execute these strings. Client operations remain a separate closed set and keep strict validation. A non-string control is invalid.

The Rust viewer decodes a run snapshot as ready, pending content, or invalid. Only a missing first snapshot or an incomplete requested content reference is pending. Missing or malformed required data is invalid and produces a safe section-level error instead of an indefinite loading screen. The error does not contain the full snapshot, session text, artifact content, or a rejected value. Local sockets and WebSockets use the same decoder. A new snapshot or completed content request replaces the cached decode result.

This changes no JSON wire value or version-1 schema identifier. It needs no state migration, compatibility reader, fallback, or alternate viewer path. See [Fix piw snapshot decoding](plans/2026-09-11-piw-snapshot-decoding-plan.md).
...
An origin-session subscription returns `pi-workflows.session-view.v1`. It contains one current workflow message, one pending interaction, the open workflow turn, and the compact current run view. Message selection reads stored message metadata only, and the server reads content for the one message Pi must act on. The compact run carries a bounded window of node rows, its first row index, and the complete row count, and its window follows the node the widget shows as working. `view.session.window` moves that window for the live session subscription and returns it to the window that follows that node. When no run is active, it keeps the most recent terminal run while its terminal workflow message is pending or its first model turn is open, and then for 60 seconds after that turn ends. Complete workflow message history stays reachable through the byte-bounded `workflow_messages` run page, whose large content arrives through the content operation.

The Pi extension applies each valid bounded session snapshot to its widget, status line, and coordinator immediately. It does not request complete run pages or hydrate large run content first. Detailed clients load that data through the same page and content operations only when they need it. This keeps the latest server snapshot as the only session-status source and prevents slow history loading from blocking newer status.

After every server connection, the extension reports the active branch before it sends a workflow message or reports a model turn. The report names the one current workflow message, or null when the session holds none yet. The extension always uses the exact run ID from that view. It does not select the latest run separately. An idle session does not create empty commands.

A snapshot page contains at most 256 items and also has a byte budget. `view.page` returns another byte-bounded window that contains the requested cursor. Page responses use `pi-workflows.run-page.v1` and echo the requested cursor and run-view revision. A client applies a page only when both still match its current request and snapshot. A step-centered trace request selects the exact stored attempt first and uses its node only when that attempt has no trace event. Large workflow topology has bounded node, edge, graph-step, and transition projections plus durable content references for the complete original definition and complete graph history. Detailed TypeScript clients assemble every run-history page for that revision and hydrate the complete definition and all referenced content before they emit a complete non-interactive view. TypeScript and Rust use the same verified content loader for complete graph steps and transitions. Rust also requests, verifies, and decodes the complete referenced definition before it builds the graph layout. A session-event page also carries the replay checkpoint for the exact sequence before its first item, so reducing the page does not lose earlier active messages or tool calls. The checkpoint can itself be a durable content reference. TypeScript hydrates it with the run view, and Rust requests and resolves it before replay.
