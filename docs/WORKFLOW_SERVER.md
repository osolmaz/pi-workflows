...
The compact run carries a bounded window of node rows, its first row index, and the complete row count. The window follows the node the widget shows as working when the subscription names no row. `view.session.window` moves the window of the live session subscription, and `null` returns it to the window that follows that node. The window is item and byte bounded, and one row's free text is bounded by the client frame budget. Complete node text and complete node history stay available through the detailed run view. Widget scrolling asks for the adjacent window, so the widget never holds the complete topology. The compact run also carries the newest progress updates for its run, up to 16 keys. Complete update history stays on the run's update page.
