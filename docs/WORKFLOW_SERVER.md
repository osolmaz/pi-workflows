...
A message that leaves the branch comes back in one of two ways. A step prompt returns as a new `resumed` message with a new identity, because a resumed prompt is a new instruction. A decision prompt returns as the same message set to `pending` with no Pi entry, because the prompt describes one decision that is still waiting and a decision cannot be answered from a message the branch no longer holds. Without that reopening the session would wait forever for an answer to a prompt it can no longer show.
