...
The response also contains the one current workflow message, which is the message Pi must inspect, add, finish, or confirm next. A cancelled step message stays current while Pi holds it and has not reported its turn, so the extension learns that a message it already delivered is cancelled and stops that turn. A cancelled step that Pi never received does not stay current: no turn needs stopping, the extension must not deliver a cancelled step again, and the retained terminal view stays visible. Message selection reads stored message metadata only, so a long session history never loads every content blob. Selection and terminal retention walk that metadata in bounded batches, in the order each step needs, and stop at the message they need. A session with two thousand stored messages therefore reads a few rows for one view call instead of the whole history. The complete history stays available through the `workflow_messages` run page. The server reads content for the selected message and sends it with its digest. A branch report can name that message ID, or null while the session holds no message yet. The server returns all these facts from one consistent read. The extension applies the bounded snapshot to the widget, status line, and coordinator immediately. It does not request run pages or hydrate workflow history first. Polling an idle session creates no durable command. Complete message history stays available through the `workflow_messages` run page and the verified content operation.
