...
Implement, test, commit, push, open a pull request, run Pi Reviewer, verify CI, and merge the approved change in this repository against `main`. Use the legacy implementation process, not a server workflow. The user's approval covers this implementation and its required validation. It does not authorize edits to other repositories, a package release, installed-runtime replacement, live-state reset, new services, credential copying, or production resource changes.
...
### Typed server-owned transitions

Replace writable full-run snapshots with narrow execution commands. Workers execute workflow code and propose attempt results, input requests, routing, and terminal results. They do not replace authoritative run state. The server checks actor, resource revision, exact attempt, ownership generation, and legal transition before committing.
...
Remove the independent scheduling and retry loop exported through `ResourceManagerRuntime`; retain shared reconciliation and effect helpers. Server scheduling and tests use the same rules.
...
2. Implement exact request identity and server-owned transition rules.
...
- Removed checkpoint continuation commands, copied steps and output overrides, continuation records, parent queue settlement, and settings transfer. Only explicit fresh restarts can declare run ancestry. Accepted responses remain discoverable for scheduler recovery after a server interruption.
...
- Model submissions and updates now name an opaque `requestId`, not a step/attempt pair. Checkpoint answers and human commands also require an exact request ID. The server and headless paths use the same contract. Old tool shapes are rejected.
