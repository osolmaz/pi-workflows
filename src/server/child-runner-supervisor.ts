...
export type SupervisedRunnerResult = {
...
type ChildRunnerSupervisorOptions<Message, Response> = {
  label: string;
...
type ChildRunnerStartOptions = {
  args: string[];
  cwd: string;
  env: NodeJS.ProcessEnv;
};

/** Shared supervision, framing, diagnostics, and process-group shutdown for server runners. */
export class ChildRunnerSupervisor<Message, Response> {
  ...
  private stopping: RunnerOutcome | null = null;
  private processing = Promise.resolve();
  private readyResolve: (() => void) | undefined;
  private readyReject: ((error: Error) => void) | undefined;
  private exitResolve: ((result: SupervisedRunnerResult) => void) | undefined;
  private readonly readyPromise = new Promise<void>((resolve, reject) => {
    this.readyResolve = resolve;
    this.readyReject = reject;
  });
  private readonly exitPromise = new Promise<SupervisedRunnerResult>((resolve) => {
    this.exitResolve = resolve;
  });

  constructor(private readonly options: ChildRunnerSupervisorOptions<Message, Response>) {
    this.decoder = new NdjsonFrameDecoder(options.maxMessageBytes, options.oversizedMessage);
  }

  async start(options: ChildRunnerStartOptions): Promise<void> {
  ...
  private onExit(code: number | null, signal: NodeJS.Signals | null): void {
    ...
    const result: SupervisedRunnerResult = {
      outcome,
      exitCode: code,
      signal,
