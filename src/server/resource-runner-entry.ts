...
import { jsonFingerprint } from "../resource-managers/json.js";
import { loadResourceManagerFile } from "../resource-managers/loader.js";
import { createResultHelpers } from "../resource-managers/results.js";
import type {
  ChildWorkflowRecord,
...
import {
  RESOURCE_RUNNER_LAUNCH_SCHEMA,
  encodeResourceRunnerLine,
  parseResourceRunnerResponse,
  type ResourceRunnerLaunchEnvelope,
  type ResourceRunnerMessage,
  type ResourceRunnerOperation,
  type ResourceRunnerResponse,
} from "./resource-runner-protocol.js";

const STARTUP_ENV = "PI_WORKFLOWS_RESOURCE_RUNNER_LAUNCH";
const MAX_FRAME_BYTES = 1024 * 1024;

class ResourceRunnerTransport {
  private readonly pending = new Map<
    string,
    { resolve: (response: ResourceRunnerResponse) => void; reject: (error: Error) => void }
  >();
  private buffered = Buffer.alloc(0);

  constructor(private readonly launch: ResourceRunnerLaunchEnvelope) {
  ...
  async request<T>(operation: ResourceRunnerOperation, payload: JsonValue): Promise<T> {
    ...
    if (response.outcome !== "accepted") {
      throw new Error(response.error ?? `Resource runner ${response.outcome}`);
    }
  ...
  close(): void {
    ...
    this.failAll(new Error("Resource runner transport closed"));
  }

  private async send(message: ResourceRunnerMessage): Promise<ResourceRunnerResponse> {
    const response = new Promise<ResourceRunnerResponse>((resolve, reject) => {
      this.pending.set(message.messageId, { resolve, reject });
    });
    if (!process.stdout.write(encodeResourceRunnerLine(message))) {
  ...
  private onData(chunk: Buffer): void {
    ...
    if (this.buffered.byteLength > MAX_FRAME_BYTES && !this.buffered.includes(0x0a)) {
      this.failAll(new Error("Resource runner response exceeds 1 MiB"));
      return;
    }
    for (;;) {
      ...
      try {
        const response = parseResourceRunnerResponse(frame);
        const pending = this.pending.get(response.messageId);
        if (pending === undefined) throw new Error("Resource runner response is unexpected");
...
class ServerBackedResourceManagerEffects implements ResourceManagerEffects {
  private used = false;

  constructor(
    private readonly transport: ResourceRunnerTransport,
...
class ServerBackedResourceManagerWorkflows implements ResourceManagerWorkflows {
  constructor(private readonly transport: ResourceRunnerTransport) {}
  ...
  async changeSettings(
    ...
    return await this.transport.request<ResourceManagerSettingsChangeResult>(
  ...
  async queueFollowUp(
    ...
    return await this.transport.request<ResourceManagerFollowUpResult>(
...
export async function runResourceRunner(): Promise<number> {
  const launch = readLaunchEnvelope();
  const transport = new ResourceRunnerTransport(launch);
  try {
    const definition = await loadResourceManagerFile(launch.definitionPath);
    if (definition.name !== launch.resourceManagerName) {
      throw new Error(
        `ResourceManager source name changed: expected ${launch.resourceManagerName}, got ${definition.name}`,
      );
    }
    await transport.request("runner.ready", {});
    ...
    try {
      const context: ReconcileContext<unknown> = {
        signal: abort.signal,
        effects: new ServerBackedResourceManagerEffects(transport, abort.signal),
        workflows: new ServerBackedResourceManagerWorkflows(transport),
...
function readLaunchEnvelope(): ResourceRunnerLaunchEnvelope {
  const encoded = process.env[STARTUP_ENV];
  if (encoded === undefined) throw new Error("Resource runner launch envelope is missing");
  const value = parseJson(Buffer.from(encoded, "base64url").toString("utf8"));
  if (
    typeof value !== "object" ||
    value === null ||
    Array.isArray(value) ||
    (value as { schema?: unknown }).schema !== RESOURCE_RUNNER_LAUNCH_SCHEMA
  ) {
    throw new Error("Resource runner launch envelope is invalid");
  }
  return value as unknown as ResourceRunnerLaunchEnvelope;
}

function validateResult(value: unknown): asserts value is ReconcileResult<unknown> {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new Error("ResourceManager reconcile result must be an object");
  }
  const result = value as { kind?: unknown; afterMs?: unknown };
  if (result.kind !== "settled" && result.kind !== "requeue") {
    throw new Error("ResourceManager reconcile result kind is invalid");
  }
  if (
    result.afterMs !== undefined &&
    (!Number.isSafeInteger(result.afterMs) || (result.afterMs as number) < 0)
  ) {
    throw new Error("ResourceManager reconcile afterMs must be a non-negative safe integer");
  }
}
...
async function main(): Promise<void> {
  try {
    process.exitCode = await runResourceRunner();
  } catch (error) {
