...
/**
 * Deterministic workflow-shape generators shared by the graph property tests
 * and the layout fixture exporter. Fixtures exported from these generators
 * pin the layout and render algorithms across implementations (the TypeScript
 * viewer and the Rust TUI), so any behavior change here is a parity break and
 * must be regenerated on both sides.
 */
...
export function randomSnapshot(seed: number): WorkflowDefinitionSnapshot {
...
export function makeRandomRunState(
  snapshot: WorkflowDefinitionSnapshot,
