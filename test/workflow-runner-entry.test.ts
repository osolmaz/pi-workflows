...
import {
  executeRunnerRunCommand,
  validateAcceptedAssistantSubmission,
} from "../src/server/workflow-runner-entry.js";
...
describe("workflow runner run commands", () => {
  it("starts a restart as a fresh run instead of a continuation", async () => {
    ...
    await expect(
      executeRunnerRunCommand(engine, workflow, "restart-1", source, {
        kind: "restart",
        input: { task: "again" },
      }),
...
describe("workflow runner assistant interaction validation", () => {
